...
async function train() {
  ...
  model = tf.sequential({
    layers: [
	    ...
	    tf.layers.dense({units: 128, activation: 'relu'}), // Hidden layer
	    tf.layers.dense({units: 64, activation: 'relu'}), // Hidden layer
  ...
  const optimizer = tf.train.adam(0.0001); // Optimizer dengan learning rate 0.0001
